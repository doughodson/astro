...
{
   return std::log(xval + std::sqrt(xval * xval + 1.0));
}  // end asinh
...
void newtonnu(double ecc, double nu, double& e0, double& m)
{
   ...
   // -------------------- hyperbolic  --------------------
   if (ecc > 1.0 + small) {
      if ((ecc > 1.0) && (std::fabs(nu) + 0.00001 < pi - std::acos(1.0 / ecc))) {
...
void rv2coe(double r[3], double v[3], double mu, double& p, double& a, double& ecc, double& incl, double& omega,
      double& argp, double& nu, double& m, double& arglat, double& truelon, double& lonper)
{
   ...
   if (magh > small) {
      ...
      std::strcpy(typeorbit, "ei");
      if (ecc < small) {
         // ----------------  circular equatorial ---------------
         if ((incl < small) | (std::fabs(incl - pi) < small))
            std::strcpy(typeorbit, "ce");
         else
            // --------------  circular inclined ---------------
            std::strcpy(typeorbit, "ci");
      } else {
         // - elliptical, parabolic, hyperbolic equatorial --
         if ((incl < small) | (std::fabs(incl - pi) < small))
            std::strcpy(typeorbit, "ee");
      }
      ...
      if ((ecc > small) && (std::strcmp(typeorbit, "ee") == 0)) {
         temp = ebar[0] / ecc;
         if (std::fabs(temp) > 1.0)
            temp = sgn(temp);
      ...
      if ((magr > small) && (std::strcmp(typeorbit, "ce") == 0)) {
         temp = r[0] / magr;
         if (std::fabs(temp) > 1.0)
            temp = sgn(temp);
...
{
   jd = 367.0 * year - std::floor((7 * (year + std::floor((mon + 9) / 12.0))) * 0.25) + std::floor(275 * mon / 9.0)
         + day + 1721013.5 + ((sec / 60.0 + minute) / 60.0 + hr) / 24.0;  // ut in days

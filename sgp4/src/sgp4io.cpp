      ...
      gravconsttype whichconst, double& startmfe, double& stopmfe, double& deltamin, elsetrec& satrec)
{
   ...
   std::sscanf(longstr1, "%2d %5ld %1c %10s %2d %12lf %11lf %7lf %2d %7lf %2d %2d %6ld ", &cardnumb, &satrec.satnum,
         &classification, intldesg, &satrec.epochyr, &satrec.epochdays, &satrec.ndot, &satrec.nddot, &nexp,
         &satrec.bstar, &ibexp, &numb, &elnum);

   if (typerun == 'v')  // run for specified times from the file
         {
      if (longstr2[52] == ' ')
         std::sscanf(longstr2, "%2d %5ld %9lf %9lf %8lf %9lf %9lf %10lf %6ld %lf %lf %lf \n", &cardnumb, &satrec.satnum,
               &satrec.inclo, &satrec.nodeo, &satrec.ecco, &satrec.argpo, &satrec.mo, &satrec.no, &revnum, &startmfe,
               &stopmfe, &deltamin);
      else
         std::sscanf(longstr2, "%2d %5ld %9lf %9lf %8lf %9lf %9lf %11lf %6ld %lf %lf %lf \n", &cardnumb, &satrec.satnum,
               &satrec.inclo, &satrec.nodeo, &satrec.ecco, &satrec.argpo, &satrec.mo, &satrec.no, &revnum, &startmfe,
               &stopmfe, &deltamin);
   } else  // simply run -1 day to +1 day or user input times
   {
      if (longstr2[52] == ' ')
         std::sscanf(longstr2, "%2d %5ld %9lf %9lf %8lf %9lf %9lf %10lf %6ld \n", &cardnumb, &satrec.satnum,
               &satrec.inclo, &satrec.nodeo, &satrec.ecco, &satrec.argpo, &satrec.mo, &satrec.no, &revnum);
      else
         std::sscanf(longstr2, "%2d %5ld %9lf %9lf %8lf %9lf %9lf %11lf %6ld \n", &cardnumb, &satrec.satnum,
               &satrec.inclo, &satrec.nodeo, &satrec.ecco, &satrec.argpo, &satrec.mo, &satrec.no, &revnum);
   }
   ...
   if ((typerun != 'v') && (typerun != 'c')) {
      ...
      if (typeinput == 'e') {
         std::printf("input start prop year mon day hr min sec \n");
         // make sure there is no space at the end of the format specifiers in scanf!
         std::scanf("%i %i %i %i %i %lf", &startyear, &startmon, &startday, &starthr, &startmin, &startsec);
         std::fflush(stdin);
         jday(startyear, startmon, startday, starthr, startmin, startsec, jdstart);

         std::printf("input stop prop year mon day hr min sec \n");
         std::scanf("%i %i %i %i %i %lf", &stopyear, &stopmon, &stopday, &stophr, &stopmin, &stopsec);
         std::fflush(stdin);
         ...
         std::printf("input time step in minutes \n");
         std::scanf("%lf", &deltamin);
      }
      // -------- enter start/stop year and days of year values -----------
      if (typeinput == 'd') {
         std::printf("input start year dayofyr \n");
         std::scanf("%i %lf", &startyear, &startdayofyr);
         std::printf("input stop year dayofyr \n");
         std::scanf("%i %lf", &stopyear, &stopdayofyr);
      ...
      if (typeinput == 'm') {
         std::printf("input start min from epoch \n");
         std::scanf("%lf", &startmfe);
         std::printf("input stop min from epoch \n");
         std::scanf("%lf", &stopmfe);
         std::printf("input time step in minutes \n");
         std::scanf("%lf", &deltamin);
      }
   }
   ...
   sgp4init(whichconst, opsmode, satrec.satnum, satrec.jdsatepoch - 2433281.5, satrec.bstar, satrec.ecco, satrec.argpo,
         satrec.inclo, satrec.mo, satrec.no, satrec.nodeo, satrec);
} // end twoline2rv
